/*
  003_version_baseline_correction_test.sql — manual acceptance test for
  supabase/migrations/003_correct_version_baseline_convention.sql.

  Supersedes 002_version_baseline_test.sql, which tested the WRONG
  convention (new rows at version=0, first event base_version=0) — that
  assumption was itself the mistake corrected by migration 003. This file
  tests the real convention: new rows start at version=1, matching what
  the client actually sends as its first event's base_version (every save
  function increments s.version BEFORE calling recordEvent() — see 003's
...
declare
  ...
  v_result jsonb;
  v_client_a text := 'Z92-Z'; -- throwaway anonymous IDs, unlikely to collide with real data even without rollback
  v_client_b text := 'Z93-Z';
...
begin
  ...
  -- ============================================================
  -- TEST 1: brand-new session, first event, base_version = 1
  -- (matches the real client convention — see file header)
  -- ============================================================
  ...
  if (v_result->>'version')::int <> 2 then
    raise exception 'TEST 1 FAILED: expected canonical version=2, got %', v_result->>'version';
  end if;
  ...
  raise notice 'TEST 1 PASSED: new session, first event (base_version 1) -> ok, version 2, 1 session_events row, 0 conflicts';

  -- ============================================================
  -- TEST 2: second sequential event, base_version = 2
  -- ============================================================
  v_result := apply_session_event(
  ...
  if (v_result->>'version')::int <> 3 then
    raise exception 'TEST 2 FAILED: expected canonical version=3, got %', v_result->>'version';
  end if;
  ...
  raise notice 'TEST 2 PASSED: second event (base_version 2) -> ok, version 3, second session_events row exists';

  -- ============================================================
  -- TEST 3: stale event — server is at version 3, event claims base 2
  -- ============================================================
  v_result := apply_session_event(
  ...
  raise notice 'TEST 3 PASSED: stale event (base_version 2 vs server 3) -> structured version_mismatch, conflict row inserted, no extra session_events row';
  ...
  begin
    ...
    -- DIFFERENT (stale/wrong) session id, with base_version=2 matching
